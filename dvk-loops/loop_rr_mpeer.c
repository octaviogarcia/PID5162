...
void  main ( int argc, char *argv[] )
{
	...
	if(  f == peers  )	{		/* MAIN  */
		...
		for (f = 0; f < peers; ){
			ret = dvk_getprocinfo(dcid, CHILD + f, p_ptr);
			printf("dvk_getprocinfo ret=%d\n",ret);
		...
		for (f = 0; f < peers; f++)
    		ret = dvk_send(CHILD+f, (long) m_ptr);
		t_start = dwalltime();
