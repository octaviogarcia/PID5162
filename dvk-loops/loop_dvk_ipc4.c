...
void  main ( int argc, char *argv[] )
{
	...
	if( (src_pid = fork()) != 0 )	{		/* PARENT = DESTINATION */
		...
		if( dst_ep < EDVSERRCODE) ERROR_PRINT(dst_ep);
		...
		if( src_ep < EDVSERRCODE) ERROR_PRINT(src_ep);
		...
		if (m_ptr== NULL) ERROR_PRINT(-errno);
		printf("m_ptr %p\n",m_ptr);
		...
		for( i = 0; i < (loops+10); i++) {
			...
			if (ret < 0 ) ERROR_PRINT(ret);
		    ret = dvk_reply(src_ep, (long) m_ptr);
			if (ret < 0 ) ERROR_PRINT(ret);
		}
     	t_stop  = dwalltime();
		...
		for( i = 0; i < (loops+10); i++){
	    	ret = dvk_sendrec(dst_ep, (long) m_ptr);
		}
