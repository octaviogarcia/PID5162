...
int udp_ioctl (int fd, ioreq_t req)
{
	...
	switch(req)	{
		...
		case NWIOUDPPEEK:
			...
			break;
		default:
			reply_thr_get(udp_fd, EDVSBADIOCTL, TRUE);
			result= NW_OK;
			break;
	}
...
int udp_read (int fd, mnx_size_t count)
{
	...
	if (!(udp_fd->uf_flags & UFF_OPTSET)){
		reply_thr_put(udp_fd, EDVSBADMODE, FALSE);
		return NW_OK;
	}
...
int udp_setopt(udp_fd_t *udp_fd)
{
	...
	if (new_en_flags & new_di_flags){
		DBLOCK(1, printf("returning EDVSBADMODE\n"));

		reply_thr_get(udp_fd, EDVSBADMODE, TRUE);
		return NW_OK;
	}
...
int udp_peek (udp_fd_t *udp_fd)
{
	...
	if (!(udp_fd->uf_flags & UFF_OPTSET)){
		udp_fd->uf_flags &= ~UFF_IOCTL_IP;
		reply_thr_put(udp_fd, EDVSBADMODE, TRUE);
		return NW_OK;
	}
...
int udp_write(int fd, mnx_size_t count)
{
	...
	if (!(udp_fd->uf_flags & UFF_OPTSET))	{
		reply_thr_get (udp_fd, EDVSBADMODE, FALSE);
		return NW_OK;
	}
...
int udp_cancel(int fd, int which_operation)
{
	...
	switch (which_operation){
		case SR_CANCEL_READ:
			...
			udp_fd->uf_flags &= ~UFF_READ_IP;
			reply_thr_put(udp_fd, EDVSINTR, FALSE);
			break;
		case SR_CANCEL_WRITE:
			...
			reply_thr_get(udp_fd, EDVSINTR, FALSE);
			break;
		case SR_CANCEL_IOCTL:
			assert (udp_fd->uf_flags & UFF_IOCTL_IP);
			udp_fd->uf_flags &= ~UFF_IOCTL_IP;
			udp_fd->uf_flags &= ~UFF_PEEK_IP;
			reply_thr_get(udp_fd, EDVSINTR, TRUE);
			break;
		default:
