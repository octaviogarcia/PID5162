...
int ip_ioctl (int fd, ioreq_t req)
{
	...
	switch (req) 	{
		case NWIOSIPOPT:
			...
			if (new_en_flags & new_di_flags){
				bf_afree(data);
				reply_thr_get (ip_fd, EDVSBADMODE, TRUE);
				return NW_OK;
			}
	...
	DBLOCK(1, printf("replying EDVSBADIOCTL: 0x%x\n", req));
	return (*ip_fd->if_put_userdata)(ip_fd->if_srfd, EDVSBADIOCTL,
		(acc_t *)0, TRUE);
}
