...
int tcp_ioctl (int fd, ioreq_t req)
{
	...
	switch (req){
		...
		default:
			tcp_fd->tf_flags &= ~TFF_IOCTL_IP;
			reply_thr_get(tcp_fd, EDVSBADIOCTL, TRUE);
			result= NW_OK;
			break;
	}
	return result;
...
int tcp_setconf(tcp_fd_t *tcp_fd)
{
	...
	if (new_en_flags & new_di_flags){
		tcp_fd->tf_flags &= ~TFF_IOCTL_IP;
		reply_thr_get(tcp_fd, EDVSBADMODE, TRUE);
		bf_afree(data);
		return NW_OK;
	}
...
tcp_conn_t *find_empty_conn(void )
{
	...
	for (i=tcp_conf_nr, tcp_conn= tcp_conn_table+i;
		...
		if (tcp_conn->tc_senddis > get_time())
			continue;
		if (tcp_conn->tc_state != TCS_CLOSED){
			 tcp_close_connection (tcp_conn, EDVSNOCONN);
		}
		tcp_conn->tc_flags= 0;
		return tcp_conn;
	}
...
tcp_conn_t *find_best_conn(mnx_ip_hdr_t *ip_hdr, tcp_hdr_t *tcp_hdr)
{
	...
	if (best_conn){
		...
		if (best_conn->tc_state != TCS_CLOSED)
			tcp_close_connection(best_conn, EDVSNOCONN);
	...
	switch (which_operation){
		case SR_CANCEL_WRITE:
			...
			if (tcp_fd->tf_write_offset)
				...
				reply_thr_get (tcp_fd, EDVSINTR, FALSE);
			break;
		case SR_CANCEL_READ:
			assert (tcp_fd->tf_flags & TFF_READ_IP);
			tcp_fd->tf_flags &= ~TFF_READ_IP;
			if (tcp_fd->tf_read_offset)
				...
				reply_thr_put (tcp_fd, EDVSINTR, FALSE);
			break;
		case SR_CANCEL_IOCTL:
			...
			if (tcp_fd->tf_flags & TFF_IOC_INIT_SP)	{
				tcp_fd->tf_flags &= ~TFF_IOC_INIT_SP;
				reply_thr_put (tcp_fd, EDVSINTR, TRUE);
				break;
			}
			...
			switch (tcp_fd->tf_ioreq)	{
				...
				case NWIOSTCPCONF:
				case NWIOTCPSHUTDOWN:
					reply_thr_get (tcp_fd, EDVSINTR, TRUE);
					break;
				case NWIOTCPCONN:
				case NWIOTCPLISTEN:
					assert (tcp_conn->tc_connInprogress);
...
int tcp_connect(tcp_fd_t *tcp_fd)
{
	...
	if (!(tcp_fd->tf_flags & TFF_CONF_SET))	{
		tcp_reply_ioctl(tcp_fd, EDVSBADMODE);
		return NW_OK;
	}
...
void tcp_buffree (int priority)
{
	...
	if (priority == TCP_PRI_CONNwoUSER){
		for (i=0, tcp_conn= tcp_conn_table; i<TCP_CONN_NR; i++,
			...
			tcp_close_connection (tcp_conn, EDVSOUTOFBUFS);
		}
	}
...
void tcp_notreach(tcp_conn_t *tcp_conn)
{
	...
	if (new_ttl == IP_MAX_TTL){
		if (tcp_conn->tc_state == TCS_SYN_SENT)
			tcp_close_connection(tcp_conn, EDVSDSTNOTRCH);
		return;
