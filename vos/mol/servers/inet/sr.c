...
message *m;
{
	...
	if (mq != NULL)	{
		...
		result= dvk_send(mq->mq_mess.m_source, &mq->mq_mess);
		if (result != OK)
...
void sr_reply_(mq_t *mq,int status,int is_revive)
{
	...
	if (is_revive)	{
		dvk_notify(mq->mq_mess.m_source);
		result= EDVSLOCKED;
	} else
		result= dvk_send(mq->mq_mess.m_source, mp);
...
void sr_select_res(int fd, unsigned ops)
{
	...
	dvk_notify(sr_fd->srf_select_proc);
}
...
//		vir_cp_req[i].dst.segment = D;
		...
		if (i == CPVEC_NR || acc == NULL)	{
			...
			if (dvk_sendrec(SYSTASK(local_nodeid), &mess) <0)
				ip_panic(("unable to dvk_sendrec("));
			if (mess.m_type <0)	{
...
int sr_repl_queue(int proc, int ref, int operation)
{
	...
	for (m= repl_queue; m;)	{
		...
		result= dvk_send(m->mq_mess.m_source, &m->mq_mess);
		if (result != OK)
	...
	repl_queue= NULL;
	if (m_cancel) {
		result= dvk_send(m_cancel->mq_mess.m_source, &m_cancel->mq_mess);
		if (result != OK)
			ip_panic(("unable to send: %d", result));
		mq_free(m_cancel);
		return 1;
