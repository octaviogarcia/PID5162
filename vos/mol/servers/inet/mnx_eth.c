...
void osdep_eth_init(void)
{
	...
	for (i= 0, ecp= eth_conf, eth_port= eth_port_table;
		...
		if (tasknr == ANY)
			...
			r= dvk_send(eth_port->etp_osdep.etp_task, &mess);
			if (r<0) {
				printf(
		"osdep_eth_init: unable to dvk_send to ethernet task, error= %d\n",
					r);
			}else{
				m_ptr = &mess;
		...
		if (r == OK){
			r= dvk_receive(eth_port->etp_osdep.etp_task, &mess);
			if (r<0) {
				printf(
	"osdep_eth_init: unable to dvk_receive from ethernet task, error= %d\n",
					r);
			}else{
				m_ptr = &mess;
...
void eth_write_port(eth_port_t *eth_port, acc_t *pack)
{
	...
	for (;;) {
		...
		r= dvk_sendrec(eth_port->etp_osdep.etp_task, &mess1);
		if (r != EDVSLOCKED)
			break;

		/* ethernet task is dvk_sending to this task, I hope */
		m_ptr = &block_msg;
		r= dvk_receive(eth_port->etp_osdep.etp_task, &block_msg);
		if (r < 0)
			ip_panic(("unable to dvk_receive"));
		SVRDEBUG(MSG2_FORMAT, MSG2_FIELDS(m_ptr));
	...
	if (r < 0){
		printf("eth_write_port: dvk_sendrec to %d failed: %d\n",
...
void eth_check_drivers(message *m)
{
	...
	r= dvk_sendrec(tasknr, m);
	if (r != OK){
		printf("eth_check_drivers: dvk_sendrec to %d failed: %d\n",
			tasknr, r);
		return;
	}
	SVRDEBUG(MSG1_FORMAT, MSG1_FIELDS(m));
...
int eth_get_stat(eth_port_t *eth_port, eth_stat_t *eth_stat)
{
	...
	for (;;) {
		r= dvk_sendrec(eth_port->etp_osdep.etp_task, &mess);
		if (r != EDVSLOCKED)
			break;

		r= dvk_receive(eth_port->etp_osdep.etp_task, &mlocked);
		mol_assert(r == OK);
	...
	if (r != OK) {
		printf("eth_get_stat: dvk_sendrec to %d failed: %d\n",
			eth_port->etp_osdep.etp_task, r);
		return EIO;
	}
...
void eth_set_rec_conf (eth_port_t *eth_port, u32_t flags)
{
	...
	do {
		r= dvk_sendrec(eth_port->etp_osdep.etp_task, &mess);
		if (r == EDVSLOCKED) {
			/* etp_task is dvk_sending to this task,  I hope */
			if (dvk_receive (eth_port->etp_osdep.etp_task, 
				&repl_mess)< 0) {
				ip_panic(("unable to dvk_receive"));
			}
	...
	if (r < 0) {
		printf("eth_set_rec_conf: dvk_sendrec to %d failed: %d\n",
			eth_port->etp_osdep.etp_task, r);
		return;
	}
...
void setup_read( eth_port_t *eth_port)
{
	...
	do 	{
		...
		for (;;) {
			SVRDEBUG("eth%d: dvk_sending DL_READV\n",mess1.DL_PORT);
			m_ptr = &mess1;
			SVRDEBUG(MSG2_FORMAT, MSG2_FIELDS(m_ptr));
			r= dvk_sendrec(eth_port->etp_osdep.etp_task, &mess1);
			SVRDEBUG(MSG2_FORMAT, MSG2_FIELDS(m_ptr));
		...
		if (r < 0)	{
			printf("mnx_eth`setup_read: dvk_sendrec to %d failed: %d\n",
				eth_port->etp_osdep.etp_task, r);
...
void eth_restart(eth_port_t *eth_port,int tasknr)
{
	...
	/* YYY */
	if (r<0) {
		printf("eth_restart: dvk_sendrec to ethernet task %d failed: %d\n",
			eth_port->etp_osdep.etp_task, r);
		return;
	}
