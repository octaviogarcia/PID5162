...
#define WAIT4BIND_MS	1000
	do { 
		...
		SVRDEBUG("WEBSRV: dvk_wait4bind_T  rcode=%d\n", rcode);
		if (rcode == EDVSTIMEDOUT) {
			SVRDEBUG("WEBSRV: dvk_wait4bind_T TIMEOUT\n");
			continue ;
		}else if( rcode < 0) 
...
int main (int argc, char *argv[] )
{
	...
	dvk_unbind(dc_ptr->dc_dcid,nweb_ptr->p_endpoint);
	...
	for(hit=1; ;hit++) {
		...
		if((pid = fork()) < 0) {
			...
			if(pid == 0) { 	/* child */
				(void)close(listenfd);
				rcode = dvk_bind(dc_ptr->dc_dcid,nweb_ptr->p_endpoint);
				if ( rcode < 0) ERROR_EXIT(rcode);
				...
				fclose(fp);
				rcode = dvk_unbind(dc_ptr->dc_dcid,nweb_ptr->p_endpoint);
				if ( rcode < 0) ERROR_EXIT(rcode);
				exit(1);
			} else { 	/* parent */
				(void)close(socketfd);
