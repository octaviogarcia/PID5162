...
int do_getsysinfo()
{
  ...
  if ((s = dvk_vcopy(SELF, src_addr, who_e, dst_addr, len)) < 0)
    ERROR_RETURN(s);
  return (OK);
 ...
 */
  ...
  /* dup2(fd, fd2) */
  if (m_in.fd2 < 0 || m_in.fd2 >= MNX_OPEN_MAX) return(EDVSBADF);
...
int do_fcntl()
{
  ...
  switch (m_in.request) {
		...
		case F_FREESP:	{
			...
			if ((r = dvk_vcopy(who_e, m_in.name1, SELF, &flock_arg, sizeof(flock_arg))) != OK)
				ERROR_RETURN(r);
...
int do_set()
{
  ...
  if (who_e != PM_PROC_NR) return (EDVSGENERIC);
...
int do_svrctl()
{
  ...
  case FSSIGNON: {
    ...
    if ((r = dvk_vcopy(who_e, m_in.svrctl_argp,
                          FS_PROC_NR, &device,
                          sizeof(device))) < 0)
		ERROR_RETURN(r);
