...
/* Perform the advisory locking required by POSIX. */
 ...
 //  r = sys_datacopy(who_e, (vir_bytes) user_flock,
  ...
  r = dvk_vcopy(who_e, user_flock, FS_PROC_NR, &flock, sizeof(flock));    
  ...
  for (flp = &file_lock[0]; flp < & file_lock[NR_LOCKS]; flp++) {
	...
	if (nr_locks == NR_LOCKS) return(EDVSNOLCK);
  ...
  if (req == F_GETLK) {
	...
	// 	who_e, (vir_bytes) user_flock, (phys_bytes) sizeof(flock));
 	r = dvk_vcopy(FS_PROC_NR, &flock, who_e, user_flock, sizeof(flock));	
    if( r < 0) ERROR_RETURN(r);
	return(OK);
  }
