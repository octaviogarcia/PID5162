...
/* Perform the chown(name, owner, group) system call. */
  ...
  if (super_user) {
    ...
    if (rip->i_uid != fp->fp_effuid) r = EDVSPERM;
    if (rip->i_uid != m_in.owner) r = EDVSPERM;  /* no giving away */
    if (fp->fp_effgid != m_in.group) r = EDVSPERM;
  }
  }
  if (r == OK) {
 ...
 */
  ...
  bits = rip->i_mode;
  test_uid = (call_nr == EDVSACCES ? fp->fp_realuid : fp->fp_effuid);
  test_gid = (call_nr == EDVSACCES ? fp->fp_realgid : fp->fp_effgid);
  if (test_uid == SU_UID) {
