...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
 ...
 // SVRDEBUG("m_in.nbytes %d\n", m_in.nbytes);
  ...
  if (m_in.nbytes < 0) return(EDVSINVAL);
...
SVRDEBUG("READING get_block(dev=%d, b=%d, n=%d) \n", dev, b, n);  
  ...
  /* Copy a chunk from the block buffer to user space. */
	//dvk_vcopy(who_e, path, FS_PROC_NR, user_path, len);  	
     //r = dvk_vcopy(FS_PROC_NR, D, usr, seg, chunk);
     r = dvk_vcopy(FS_PROC_NR, (bp->b_data+off), usr, buff, chunk);
SVRDEBUG("Resultado dvk_vcopy SERVIDOR -> CLIENTE %d\n", r);  
SVRDEBUG("chunk %d\n", chunk);     
     //SVRDEBUG("buff %s\n", buff);
// SVRDEBUG("bp->b_data+off %s\n", bp->b_data+off);
  ...
  //     (phys_bytes) chunk);
     //r = dvk_vcopy(usr, seg, FS_PROC_NR, D, chunk);         
     r = dvk_vcopy(usr, buff, FS_PROC_NR, (bp->b_data+off), chunk);  
	 SVRDEBUG("Resultado dvk_vcopy CLIENTE -> SERVIDOR %d\n", r);  
	 if( r >= 0 ) r = OK;
	 SVRDEBUG("chunk %d\n", chunk);  
//SVRDEBUG("buff %s\n", buff);
// SVRDEBUG("bp->b_data+off %s\n", bp->b_data+off);
