...
pid_t do_fork(void)
{
    ...
    if ((child_lpid = fork()) == 0) {     /* first child */	
		do { 
			...
			SVRDEBUG("CHILD: dvk_wait4bind_T  rcode=%d\n", rcode);
			if (rcode == EDVSTIMEDOUT) {
				SVRDEBUG("CHILD: dvk_wait4bind_T TIMEOUT\n");
				continue ;
			}else if( rcode < 0) 
				exit(EXIT_FAILURE);
		} while	(rcode < OK);
...
int main ( int argc, char *argv[] )
{
	...
	get_dvs_params();
	SVRDEBUG("INIT: dvk_wait4bind_T\n");
	do { 
		rcode = dvk_wait4bind_T(FORK_WAIT_MS);
		SVRDEBUG("INIT: dvk_wait4bind_T  rcode=%d\n", rcode);
		if(rcode > 0) break;
		if (rcode != EDVSTIMEDOUT) {
			ERROR_EXIT(rcode);
			continue ;
		}
		SVRDEBUG("INIT: dvk_wait4bind_T TIMEOUT\n");
...
#define SINGLE_SERVER 	10 	/* endpoint of remote SINGLE SERVER */
			...
			rcode = dvk_sendrec(SINGLE_SERVER , (long) m_ptr);
			if(rcode < 0) {
				fprintf(stderr,"CLIENT: dvk_sendrec rcode=%d\n", rcode);
			}
			printf(MSG1_FORMAT, MSG1_FIELDS(m_ptr));
			free(m_ptr);
