...
void ftpd_init(void)
{
	...
	do { 
		...
		SVRDEBUG("M3FTPD: dvk_wait4bind_T  rcode=%d\n", rcode);
		if (rcode == EDVSTIMEDOUT) {
			SVRDEBUG("M3FTPD: dvk_wait4bind_T TIMEOUT\n");
			continue ;
		}else if( rcode < 0) 
...
int  main ( int argc, char *argv[] )
{
	...
	while(TRUE){
		...
		do { 
			...
			SVRDEBUG("M3FTPD: dvk_receive_T  rcode=%d\n", rcode);
			if (rcode == EDVSTIMEDOUT) {
				SVRDEBUG("M3FTPD: dvk_receive_T TIMEOUT\n");
				continue ;
			}else if( rcode < 0) 
				ERROR_EXIT(EXIT_FAILURE);
		} while	(rcode < OK);
		...
		switch(m_ptr->FTPOPER){
			case FTP_GET:
				...
				while( (rlen = fread(data_ptr, 1, m_ptr->FTPDLEN, fp)) > 0) {
					...
					m_ptr->FTPDLEN = rlen;
					m_ptr->m_type = OK;					
					rcode = dvk_send_T(m_ptr->m_source, m_ptr, SEND_RECV_MS);
					if(rcode < 0) {
						ERROR_PRINT(rcode); 
						break;
					}
					total_bytes += rlen;
					rcode = dvk_receive_T(m_ptr->m_source,m_ptr, SEND_RECV_MS);
					if(rcode < 0) {
						ERROR_PRINT(rcode); 
						break;
					}
			...
			case FTP_PUT:
				...
				while( m_ptr->FTPDLEN > 0) {
					rcode = dvk_vcopy(m_ptr->m_source, m_ptr->FTPDATA
								,SELF, data_ptr
								,m_ptr->FTPDLEN);
					if(rcode < 0) {
						ERROR_PRINT(rcode); 
