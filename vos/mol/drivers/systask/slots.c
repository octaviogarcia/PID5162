 ...
 *===========================================================================*/
 int primary_sts_mcast(void)
 {
	...
	if (primary_mbr == local_nodeid ){
		nr_waitsts = get_next_mbr(nr_waitsts, bm_waitsts);
		if(nr_waitsts != NO_NEXT_MBR){
			TASKDEBUG("nr_waitsts=%d\n", nr_waitsts);
			rcode = mcast_status_info(nr_waitsts, SYS_PUT_STATUS,(char*) slot);
		}
	}
	return(rcode);	
 }
 ...
 * format of mbr_string ""#xxxxxNN.yyyy" 
...
int get_nodeid(char *grp_name, char *mbr_string)
{
	...
	s_ptr = strchr(&mbr_string[len], '.'); /* locate the dot character after "#xxxxxNN.yyyy" */
	assert(s_ptr != NULL);

	*s_ptr = '\0';
	s_ptr = &mbr_string[len];
	nid = atoi(s_ptr);
...
int init_global_vars()
{
	...
	bm_nodes	= 0;	/* which nodes are ACTIVE - 			Replicated	*/
...
int init_slots(void)
{
	...
	*--------------------------------------------------------------------------------------*/
	...
	sprintf( replica_name, "MOL%02d", dc_ptr->dc_dcid);
	TASKDEBUG("replica_name=%s\n", replica_name);
	sprintf( Spread_name, "4803");
	sprintf( User, "MOL%0d.%0d", local_nodeid, dc_ptr->dc_dcid);
	TASKDEBUG("User=%s\n", User);
...
int mcast_radar_info(void)
{
	...
	msg.m2_l1	= bm_nodes;
	msg.m2_l2	= bm_init;
	msg.m2_p1	= (char *) bm_radar;
	
	TASKDEBUG("local_nodeid=%d, nr_nodes=%d, nr_init=%d\n", local_nodeid, nr_nodes, nr_init);
...
int sp_join(int new_mbr)
{
	...
	TASKDEBUG("FSM_state=%X new_member=%d primary_mbr=%d nr_nodes=%d bm_nodes=%X\n", 
		FSM_state, new_mbr, primary_mbr, nr_nodes, bm_nodes);
	
	if( new_mbr == local_nodeid){		/* The own JOIN message	*/
		if (nr_nodes == 1){ 		/* It is a LONELY member*/
...
//			return(OK);
		...
		TASKDEBUG("init_mbr=%d nr_init=%d bm_init=%X\n", init_mbr, nr_init, bm_init);
		/* compute the slot high water threshold	*/
		max_owned_slots	= (total_slots - (min_owned_slots*(nr_init-1)));
		TASKDEBUG("bm_init=%X nr_init=%d max_owned_slots=%d\n",bm_init, nr_init, max_owned_slots );
	...
	*/
	...
	TASKDEBUG( DC_USR1_FORMAT, DC_USR1_FIELDS(dcf_ptr));
		...
		if( i < dc_ptr->dc_nr_sysprocs) {
#ifdef ALLOC_LOCAL_TABLE 			
			proc_ptr = &proc[i];
			rcode  = dvk_getprocinfo(dc_ptr->dc_dcid, (i-dc_ptr->dc_nr_tasks), proc_ptr);
			if( rcode < 0) ERROR_RETURN(rcode);
#else /* ALLOC_LOCAL_TABLE */			
			proc_ptr = (proc_usr_t *) PROC_MAPPED((i-dc_ptr->dc_nr_tasks));	
			rcode = OK;			
			...
			if( TEST_BIT(proc_ptr->p_rts_flags, BIT_SLOT_FREE)) {
				if( spt->s_endpoint != NONE) {
					...
					rcode = dvk_rmtbind(dc_ptr->dc_dcid,"remote",spt->s_endpoint,spt->s_owner);
				}
				slot[i] = slot_tmp[i];
				continue;		
			} 
			...
			if( proc_ptr->p_nodeid == spt->s_owner){
				...
				if( proc_ptr->p_endpoint == spt->s_endpoint){
					...
					rcode = dvk_unbind(dc_ptr->dc_dcid, proc_ptr->p_endpoint);
					rcode = dvk_rmtbind(dc_ptr->dc_dcid,"remote",spt->s_endpoint,spt->s_owner);
				}
			}else{ 
				...
				if( proc_ptr->p_endpoint == spt->s_endpoint){
					...
					rcode = dvk_migr_start(dc_ptr->dc_dcid, proc_ptr->p_endpoint);	
					rcode = dvk_migr_commit(PROC_NO_PID, dc_ptr->dc_dcid, proc_ptr->p_endpoint, spt->s_owner);
...
int sp_don_slots(message  *spin_ptr)
{
	...
	*/
	if( owned_slots == 0 && spin_ptr->mA_nr > 0){
		...
		TASKDEBUG("Wake up systask nr_init=%d bm_init=%X\n", nr_init,bm_init);
		pthread_cond_signal(&sys_barrier);	/* Wakeup SYSTASK 		*/	
	}else{	
		free_slots+= spin_ptr->mA_nr;
		owned_slots+=spin_ptr->mA_nr;
...
int sp_disconnect(int  disc_mbr)
{
	...
	TASKDEBUG("FSM_state=%X disc_mbr=%d nr_nodes=%d\n",FSM_state, disc_mbr, nr_nodes);
	...
	if( TEST_BIT(bm_init, disc_mbr)) {
		nr_init--;	/* decrease the number of initialized nodes */
	...
	TASKDEBUG("primary_mbr=%d nr_init=%d\n",primary_mbr, nr_init);
	TASKDEBUG("bm_nodes=%X bm_init=%X bm_donors=%X\n",	bm_nodes,bm_init,bm_donors);
...
int sp_net_partition(void)
{
	...
	TASKDEBUG("bm_init=%X bm_nodes=%X bm_waitsts=%X \n", bm_init, bm_nodes, bm_waitsts);
	bm_init 	&= bm_nodes;
	bm_waitsts 	&= bm_nodes;
	
	if( FSM_state == STS_WAIT_STATUS) {
		if( bm_init == 0)
			ERROR_RETURN(EDVSNOREPLICA);
		return(OK);
	}
			...
			if( slot[i].s_owner != local_nodeid) { 
				...
				if( !TEST_BIT(bm_init, slot[i].s_owner)) {
					...
					if( TEST_BIT(proc_ptr->p_rts_flags, BIT_REMOTE))
						dvk_migr_start(dc_ptr->dc_dcid, proc_ptr->p_endpoint);
				}
			}
			continue;
		}
...
//			 && (!TEST_BIT(bm_init, slot[i].s_owner))
	...
	/* recalculate global variables */
	max_owned_slots	= (total_slots - (min_owned_slots*(nr_init-1)));
	TASKDEBUG("total_slots=%d max_owned_slots=%d nr_init=%d owned_slots=%d\n",
		total_slots,max_owned_slots, nr_init,owned_slots);
...
//	}
	...
	if( owned_slots == 0) {
		if( nr_init > 1){
			...
			TASKDEBUG("Partition without slots, exiting! nr_init=%d\n", nr_init);
			ERROR_RETURN(EDVSNOREPLICA);
		}
	}
...
int sp_merge_status(int sender, char *spin_ptr, int bytes)
{
		...
		for(i = 0; i < (dc_ptr->dc_nr_tasks + dc_ptr->dc_nr_procs); i++){
			...
			/* MERGING POLICY
				...
				2- Remotely bound slots remain Remotely bound
					a- if remain the same endpoint  (dvk_migr_rollback)
					b- if remain the same endpoint but on other remote node (dvk_migr_commit)
					c- if the endpoint changed (dvk_unbind, dvk_rmtbind)
				3- Free slots now bound on remote node (dvk_rmtbind)
			*/	
			if( i < dc_ptr->dc_nr_sysprocs) {
				...
				if( TEST_BIT(proc_ptr->p_rts_flags, BIT_SLOT_FREE)) {
					if( spm->s_endpoint != NONE) {
						rcode = dvk_rmtbind(dc_ptr->dc_dcid,"remote",spm->s_endpoint,spm->s_owner);
				...
				if( spm->s_owner == spp->s_owner){ 
					...
					if( spm->s_endpoint == spp->s_endpoint){ 
						TASKDEBUG("dvk_migr_rollback %i " SLOTS_FORMAT, i, SLOTS_FIELDS(spm));
						rcode = dvk_migr_rollback(dc_ptr->dc_dcid, proc_ptr->p_endpoint);
					}else{	/* same node , different endpoint */
						TASKDEBUG("1)unbind(%d), bind(%d), %i " SLOTS_FORMAT,
							spm->s_endpoint, spp->s_endpoint, i, SLOTS_FIELDS(spm));
						rcode = dvk_unbind(dc_ptr->dc_dcid, spm->s_endpoint);
						slot_merge[i] = slot_part[i];	/* copy the slot	*/
						rcode = dvk_rmtbind(dc_ptr->dc_dcid,"remote",spm->s_endpoint,spm->s_owner);
					}
				}else{ 
					...
					if( spm->s_endpoint == spp->s_endpoint){
						slot_merge[i] = slot_part[i];	/* copy the slot	*/	
						rcode = dvk_migr_commit(PROC_NO_PID, dc_ptr->dc_dcid, proc_ptr->p_endpoint, spm->s_owner);
						TASKDEBUG("dvk_migr_commit %i " SLOTS_FORMAT, i, SLOTS_FIELDS(spm));
					}else{ /* different  node , different endpoint */
						rcode = dvk_unbind(dc_ptr->dc_dcid, spm->s_endpoint);
	...
	/* recalculate global variables */
	max_owned_slots	= (total_slots - (min_owned_slots*(nr_init-1)));
	TASKDEBUG("bm_init=%X max_owned_slots=%d nr_init=%d\n",
		bm_init, max_owned_slots,nr_init);
...
int sp_syscall(message  *spin_ptr)
{
	...
	switch( spin_ptr->m_type) {
		...
		*-----------------------------------------------------------------------*/
		...
		case MOLFORK:
			...
			TASKDEBUG("dvk_rmtbind dcid=%d name=%s p_nr=%d p_endpoint=%d nodeid=%d\n",
				dc_ptr->dc_dcid,spin_ptr->M3_NAME, spin_ptr->M3_P_NR
				,spin_ptr->M3_ENDPT, spin_ptr->m_source);
				
			ret = dvk_rmtbind(dc_ptr->dc_dcid,spin_ptr->M3_NAME, 
				spin_ptr->M3_ENDPT, spin_ptr->m_source);
			if( ret < EDVSERRCODE) ERROR_RETURN(ret);

			/* Get info about the process from the KERNEL */
#ifdef ALLOC_LOCAL_TABLE 			
			ret = dvk_getprocinfo(dc_ptr->dc_dcid, spin_ptr->M3_P_NR, proc_ptr);
			...
			ret = dvk_getpriv(dc_ptr->dc_dcid, spin_ptr->M3_ENDPT, priv_ptr);
			if( ret < 0) ERROR_RETURN(ret);
		...
		case MOLEXEC:
			...
			/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
			/* 			SIN TERMINAR 				*/
			/* falta crear una dvk_call que permita cambiar el  	*/
		...
		case MOLMIGRPROC:
			...
			*-----------------------------------------------------------------------*/		
			...
			ret = dvk_migr_start(dc_ptr->dc_dcid, proc_ptr->p_endpoint);
			if(ret != OK)  ERROR_RETURN(ret);
			
			ret = dvk_migr_commit(PROC_NO_PID, dc_ptr->dc_dcid,  proc_ptr->p_endpoint, spin_ptr->m_source);
			if(ret != OK)  ERROR_RETURN(ret);
		...
		case MOLEXIT:
			...
			/* DeRegister  the remote process to the kernel */
			ret = dvk_unbind(dc_ptr->dc_dcid,spin_ptr->M3_ENDPT);
			if(ret != OK)  ERROR_RETURN(ret);
...
#ifdef ALLOC_LOCAL_TABLE 			
	pm_ptr = &pm_desc;
	ret = dvk_getprocinfo(dc_ptr->dc_dcid, PM_PROC_NR, (long int) pm_ptr);
	if( ret != OK) ERROR_RETURN(ret);
#else /* ALLOC_LOCAL_TABLE */			
	...
	if( pm_ptr->p_nodeid == local_nodeid){
		...
		if( (spin_ptr->M3_P_NR >= PM_PROC_NR) &&
			...
			dvk_ntfy_value(LOCAL_SLOTS, PM_PROC_NR, proc_ptr->p_nr);	
		}
	}
#endif /* ANULADO */
...
//	sp_ep = dvk_tbind(dcu.dc_dcid, sp_nr);
#define SELFTASK (-1) 
	sp_ep = dvk_replbind(dcu.dc_dcid, SELFTASK ,sp_nr);
	proc_ptr = PROC2PTR(sp_nr);
	if( sp_ep < EDVSERRCODE) ERROR_EXIT(sp_ep);
	TASKDEBUG( "has bound with p_endpoint=%d \n", sp_ep);

#ifdef ALLOC_LOCAL_TABLE 			
	rcode  = dvk_getprocinfo(dcu.dc_dcid, sp_nr, proc_ptr);
	if( rcode < 0) ERROR_RETURN(rcode );
#else /* ALLOC_LOCAL_TABLE */			
...
//				*mtype = mess_type;
				...
				ret = sp_merge_status(get_nodeid("#MOL", sender), (char *) &mess_in, ret);
				*mtype = mess_type;				
		...
		if  ( Is_reg_memb_mess( service_type ) ) {
			...
			if( Is_caused_join_mess( service_type ) ||
				...
				for( i=0; i < num_groups; i++ ){
					mbr = get_nodeid("#MOL", &sp_members[i][0]);
					TASKDEBUG("\t%s:%d\n", &sp_members[i][0], mbr );
					SET_BIT(bm_tmp, mbr);
//					TASKDEBUG("grp id is %d %d %d\n",memb_info.gid.id[0], memb_info.gid.id[1], memb_info.gid.id[2] );
				}
				TASKDEBUG("num_groups=%d OLD bm_nodes=%X NEW bm_nodes=%X\n", 
					num_groups, bm_nodes, bm_tmp);				
			}
		}
		...
		if( Is_caused_join_mess( service_type ) )	{
			...
			*----------------------------------------------------------------------------------------------------*/
			...
			if ( strncmp(memb_info.changed_member, "#RADAR",6) != 0) { // MOL member 		
				mbr = get_nodeid("#MOL", (char *)  memb_info.changed_member);
				...
				mbr = get_nodeid("#RADAR", (char *)  memb_info.changed_member);
				TASKDEBUG("JOIN - nr_radar=%d bm_radar=%X\n", nr_radar, bm_radar); 
				nr_radar = num_groups - nr_nodes;
				SET_BIT(bm_radar, mbr);
				TASKDEBUG("JOIN end - nr_radar=%d bm_radar=%X\n", nr_radar, bm_radar); 
				if( local_nodeid == primary_mbr)
					mcast_radar_info();
			}
			...
			*----------------------------------------------------------------------------------------------------*/
			...
			if ( strncmp(memb_info.changed_member, "#RADAR",6) != 0) { // RDISK member 		
				mbr = get_nodeid("#MOL", (char *)  memb_info.changed_member);
				if( !TEST_BIT(dc_ptr->dc_nodes, mbr)){
					SYSERR(EDVSNODCNODE);
					TASKDEBUG("Member %d do not belong to DC=%d\n",
				...
				mbr = get_nodeid("#RADAR", (char *)  memb_info.changed_member);
				TASKDEBUG("LEAVE - nr_radar=%d bm_radar=%X\n", nr_radar, bm_radar); 
				nr_radar = num_groups - nr_nodes;
				CLR_BIT(bm_radar, mbr);
				TASKDEBUG("LEAVE end - nr_radar=%d bm_radar=%X\n", nr_radar, bm_radar);  				
			}	
			...
			*----------------------------------------------------------------------------------------------------*/
				...
				* get the bitmap of current members
				...
				for( j = 0; j < vssets[i].num_members; j++ ) {
					...
					if ( strncmp(memb_info.changed_member, "#RADAR",6) != 0) {			
						...
						mbr = get_nodeid("#RADAR", members[j]);
						if(!TEST_BIT(bm_radar, mbr)) {
							SET_BIT(bm_radar, mbr);
							nr_radar++;
						}
					}
			...
			TASKDEBUG("OLD bm_nodes=%X nr_nodes=%d\n", bm_nodes, nr_nodes);
			TASKDEBUG("NEW bm_nodes=%X nr_nodes=%d\n", bm_tmp, nr_tmp);

			if( bm_nodes > bm_tmp) {	/* a NETWORK PARTITION has occurred 	*/
				TASKDEBUG("NETWORK PARTITION has occurred\n");
				nr_nodes = nr_tmp; 
				bm_nodes = bm_tmp;
				assert( nr_nodes > 0 && nr_nodes <= dc_ptr->dc_nr_nodes);
				sp_net_partition();
			}else{
				if (bm_nodes < bm_tmp) {	/* a NETWORK MERGE has occurred 		*/
