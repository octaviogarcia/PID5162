...
//		if( sysproc_ptr->p_rts_flags != SLOT_FREE) 	return(EDVSBUSY);
	}
	...
	rcode = dvk_migr_start(dc_ptr->dc_dcid, sysproc_ptr->p_endpoint);
	if( rcode < 0) ERROR_RETURN(rcode);
	
	rcode = dvk_migr_commit(sysproc_lpid, dc_ptr->dc_dcid, 
