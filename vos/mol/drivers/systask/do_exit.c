 ...
 */
	...
	rcode = dvk_getprocinfo(dc_ptr->dc_dcid, proc_nr, &proc[proc_nr+dc_ptr->dc_nr_tasks]);
...
//		ERROR_RETURN(EDVSBADNODEID);	
	...
	if( !TEST_BIT(proc_ptr->p_misc_flags, MIS_BIT_KILLED)){ /* NO, it exits by itself */
		...
		rcode = dvk_ntfy_value(PM_PROC_NR, proc_ep, PM_PROC_NR);
		if( rcode < 0) 	ERROR_RETURN(rcode);		
#define TO_WAIT4UNBIND	100 /* miliseconds */
		TASKDEBUG("dvk_wait4unbind_T\n");
		do { 
			rcode = dvk_wait4unbind_T(proc_ep, TO_WAIT4UNBIND);
			TASKDEBUG("dvk_wait4unbind_T  rcode=%d\n", rcode);
			if (rcode == EDVSTIMEDOUT) {
				TASKDEBUG("dvk_wait4unbind_T TIMEOUT\n");
				continue ;
			}else if( rcode < 0) 
