...
//			ret = dvk_getdcinfo(dc_ptr->dc_dcid, dc_ptr);
    	...
    	case GET_PROCTAB: {
			...
			/* it must be copied to user-space buffer because the dvk_vcopy some lines down*/
...
//	if( caller_ptr->p_rts_flags == SLOT_FREE) 	ERROR_RETURN(EDVSNOTBIND); 
	ret =	dvk_vcopy(SYSTASK(local_nodeid), src_addr, m_ptr->m_source, m_ptr->I_VAL_PTR, length);
