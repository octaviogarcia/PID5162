...
TASKDEBUG("child_ep=%d child_nodeid=%d child_name=%s\n", child_ep, child_nodeid, child_name);
	...
	child_ep  = dvk_rmtbind(vm_ptr->vm_vmid, child_name , child_ep, child_nodeid);
	TASKDEBUG("child_ep=%d\n", child_ep);
	if(child_ep < 0 && child_ep != EDVSBUSY) ERROR_RETURN(child_ep);
	
	/* Get process information from kernel */
	child_ptr = PROC2PTR(child_nr);
	rcode = dvk_getprocinfo(vm_ptr->vm_vmid, child_nr, child_ptr);
...
TASKDEBUG("child_ep=%d\n", child_ep);
	...
	rcode = dvk_getpriv(vm_ptr->vm_vmid, child_ep, cpriv_ptr);
	if( rcode < 0) ERROR_RETURN(rcode);
