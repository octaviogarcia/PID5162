...
int do_rexec(message *m_ptr)	
{
	...
	if( who_p != PM_PROC_NR) ERROR_RETURN(EDVSPERM);
	
#ifdef ALLOC_LOCAL_TABLE 			
	pm_ptr = &pm_desc;
	rcode = dvk_getprocinfo(dc_ptr->dc_dcid, PM_PROC_NR, (long int) pm_ptr);
...
#ifdef ALLOC_LOCAL_TABLE 			
	d_ptr = &d_desc;
	rcode = dvk_getprocinfo(dc_ptr->dc_dcid, thrower_ep, (long int) d_ptr);
	if( rcode != OK) ERROR_RETURN(rcode);
#else /* ALLOC_LOCAL_TABLE */			
	...
	if(pm_ptr->p_nodeid != local_nodeid) {
		...
		rcode = dvk_vcopy(thrower_ep, m_ptr->M7_ARGV_PTR, 
		...
		if(arg_c >= MNX_MAX_ARGS) ERROR_RETURN(EDVS2BIG);
		...
		if ((fork_lpid = fork()) == 0) {     
			...
			do { 
				rcode = dvk_wait4bind_T(FORK_WAIT_MS);
				TASKDEBUG("CHILD: dvk_wait4bind_T rcode=%d\n", rcode);
				if (rcode == EDVSTIMEDOUT) {
					TASKDEBUG("CHILD: dvk_wait4bind_T TIMEOUT\n");
					continue ;
				}else if( rcode < 0) 
			...
			fork_lpid = getpid();
			fork_ep = dvk_getep(fork_lpid);

			/* Wait a NOTIFY message from PM  */
			do { 		
				rcode = dvk_receive_T(PM_PROC_NR, fork_ptr, FORK_WAIT_MS);	
				TASKDEBUG("CHILD: dvk_receive_T from PM rcode=%d m_type=%d\n", rcode, fork_ptr->m_type);
				if (rcode == EDVSTIMEDOUT) {
					TASKDEBUG("CHILD: dvk_receive_T TIMEOUT\n");
					continue ;
				}else if( rcode < 0) {
					ERROR_EXIT(EXIT_FAILURE);
				}
...
//			rcode = execvp(argv_ptr, &argv_ptr);
		...
		if( fork_ep != ANY ) {
...
#ifdef ALLOC_LOCAL_TABLE 	
			f_ptr = &f_desc;
			rcode = dvk_getprocinfo(dc_ptr->dc_dcid, fork_ep, f_ptr);
			if( rcode < 0) ERROR_RETURN(rcode);
#else /* ALLOC_LOCAL_TABLE */			
			...
			rcode = dvk_lclbind(dc_ptr->dc_dcid,fork_lpid,fork_ep);
			if( rcode < 0) ERROR_RETURN(rcode);
		...
		rcode = dvk_rmtbind(dc_ptr->dc_dcid, basename(arg_v[0]), 
					fork_ep, rmt_nodeid);
		if( rcode < 0) {
			free(argv_buf);	
			ERROR_RETURN(rcode);			
