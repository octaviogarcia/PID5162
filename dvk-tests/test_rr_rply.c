...
void  main ( int argc, char *argv[] )
{
	...
	if( child_pid = fork() ) { //  PARENT
		...
		printf("CHILD FIRST RCVRQST\n");
		ret = dvk_rcvrqst((long) m_ptr);
		if( ret != 0 ) ERROR_PRINT(ret);
		printf("CHILD RCVRQST msg:" MSG1_FORMAT, MSG1_FIELDS(m_ptr));	

		printf("CHILD FIRST REPLY\n");
		m_ptr->m_type = 0xFE;
		...
		printf("CHILD SECOND RCVRQST\n");
		ret = dvk_rcvrqst((long) m_ptr);
		if( ret != 0 ) ERROR_PRINT(ret);
		printf("CHILD RCVRQST msg:" MSG1_FORMAT, MSG1_FIELDS(m_ptr));	

		printf("CHILD SECOND REPLY\n");
		m_ptr->m_type = 0xEF;
		ret = dvk_send(m_ptr->m_source,(long) m_ptr);
		if( ret != 0 ) ERROR_PRINT(ret);
	}
